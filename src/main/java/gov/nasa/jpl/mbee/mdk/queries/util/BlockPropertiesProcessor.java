...
public abstract class BlockPropertiesProcessor implements IMatchProcessor<BlockPropertiesMatch> {
   ...
   * 
   */
  public abstract void process(final com.nomagic.uml2.ext.magicdraw.classes.mdkernel.Class pBlock, final Stereotype pStereotype, final Slot pSlot, final Property pProperty, final LiteralSpecification pLiteral);
  
  @Override
  public void process(final BlockPropertiesMatch match) {
    process(match.getBlock(), match.getStereotype(), match.getSlot(), match.getProperty(), match.getLiteral());
  }
}
