...
public final class FindCommonParentClassQuerySpecification extends BaseGeneratedEMFQuerySpecification<FindCommonParentClassMatcher> {
   ...
   * 
   */
  public static FindCommonParentClassQuerySpecification instance() {
    try{
  ...
  @Override
  protected FindCommonParentClassMatcher instantiate(final ViatraQueryEngine engine) {
    return FindCommonParentClassMatcher.on(engine);
  }
  
  @Override
  public FindCommonParentClassMatcher instantiate() {
  ...
  private static class GeneratedPQuery extends BaseGeneratedEMFPQuery {
    ...
    public Set<PBody> doGetContainedBodies() {
      Set<PBody> bodies = Sets.newLinkedHashSet();
      {
          ...
          // 	find block(block1)
          new PositivePatternCall(body, Tuples.flatTupleOf(var_block1), BlockQuerySpecification.instance().getInternalQueryRepresentation());
          // 	find block(block2)
          new PositivePatternCall(body, Tuples.flatTupleOf(var_block2), BlockQuerySpecification.instance().getInternalQueryRepresentation());
          // 	block1 != block2
          ...
          new NegativePatternCall(body, Tuples.flatTupleOf(var_attribute1), RedefiningPropertyQuerySpecification.instance().getInternalQueryRepresentation());
          // 	neg find redefiningProperty(attribute2)
          new NegativePatternCall(body, Tuples.flatTupleOf(var_attribute2), RedefiningPropertyQuerySpecification.instance().getInternalQueryRepresentation());
          ...
          new PositivePatternCall(body, Tuples.flatTupleOf(var_attribute1, var_name, var_type), PropertyAttributesQuerySpecification.instance().getInternalQueryRepresentation());
          // 	find propertyAttributes(attribute2, name, type)
          new PositivePatternCall(body, Tuples.flatTupleOf(var_attribute2, var_name, var_type), PropertyAttributesQuerySpecification.instance().getInternalQueryRepresentation());
